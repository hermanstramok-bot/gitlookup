...
// ИМПОРТ YOUTUBE
// ==============================
router.post('/import/youtube', authenticateToken, async (req, res) => {
  ...
  const processSubtitles = () => {
    ...
    exec(command.cmd, { cwd: downloadDir }, (error, stdout, stderr) => {
      ...
      // Сохраняем материал в БД через Prisma
      ...
      .then(material => {
        console.log(`✓ Saved ${subtitles.length} subtitles for video ID ${material.id} (title: ${videoTitle})`);
        cleanup();
